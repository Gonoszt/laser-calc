        ...
        with tab_obj:
            ...
            with col_a:
                ...
                width = st.number_input(
                    "Szélesség (mm)", value=100.0, key=f"width{key_s}"
                )
                height = st.number_input(
                    "Magasság (mm)", value=100.0, key=f"height{key_s}"
                )
                runtime = st.number_input(
                    "Gépidő (perc)", value=float(default_gtime), key=f"time{key_s}"
                )
            with col_b:
                pcs = st.number_input(
                    "Darabszám a táblán", min_value=1, value=1, key=f"pcs{key_s}"
                )
                use_magnet = st.checkbox("Mágnes kell rá?", key=f"umag{key_s}")
                use_paint = st.checkbox(
                    "Festés / Koptatás kell?", key=f"upai{key_s}"
                )
                paint_multiplier = 1
                if use_paint:
            ...
            # Archiválás csak akkor, ha van DB
            if st.session_state.logged_in:
                if st.button(
                    "Mentés az Archívumba",
                    key=f"final_save_{key_s}",
                    use_container_width=True,
                ):
                    if not db:
                        st.error(
                            "Nincs adatbázis kapcsolat! "
                            "Az admin mód aktív, de Firestore nem elérhető."
                        )
...
# --- 2. OLDAL: SVG IDŐBECSLŐ ---
    ...
    with col1:
        v_raster = st.number_input(
            "Fekete sebesség (Raszter) [mm/s]", value=300
        )
    ...
    with col2:
        if uploaded_file:
            ...
            try:
                w_match = re.search(
                    r'width="([\d\.]+)(mm|cm|px|pt)?"', svg_raw
                )
                vb_match = re.search(
                    r'viewBox="[\d\.]+\s+[\d\.]+\s+([\d\.]+)', svg_raw
                )
            ...
            try:
                ...
                for path, attr in zip(paths, attributes):
                    ...
                    if any(c in all_p for c in ["blue", "#0000ff", "0,0,255"]):
                        ...
                        c in all_p for c in ["black", "#000000", "0,0,0"]
                    ) and "fill:none" not in all_p:
                        try:
...
# --- 3. OLDAL: ARCHÍVUM ---
    ...
    if not db:
        st.error(
            "Az Archívum megjelenítéséhez Firestore kapcsolat szükséges, "
        ...
        try:
            ...
            if res:
                ...
                selected_to_delete = st.multiselect(
                    ...
                    options=[
                        f"{row['id']} | {row['Dátum']} - {row['Gép']} - {row['Termék']}"
                        for row in res
                    ],
                )

                if selected_to_delete and st.button(
                    "Kiválasztottak törlése", type="primary"
                ):
                    try:
                        for item in selected_to_delete:
                            db.collection("kalkulaciok").document(
                                item.split(" | ")[0]
                            ).delete()
                        st.success("Törölve!")
                        st.rerun()
