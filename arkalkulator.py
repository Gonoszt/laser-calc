...
if page == "Költség Kalkulátor":
    ...
    def render_laser_tab(gep_nev, tab_obj, key_suffix):
        with tab_obj:
            ...
            # Alapértékek szerkesztése
            with st.expander(f"Alapértékek módosítása ({gep_nev})"):
                c1, c2, c3 = st.columns(3)
                lazer_f = c1.number_input(f"Lézer amort. (Ft/perc)", value=float(all_settings[gep_nev]["lazer"]), format="%.4f", key=f"l_{key_suffix}")
                mat_f = c2.number_input(f"Anyagár (Ft/mm²)", value=float(all_settings[gep_nev]["material"]), format="%.6f", key=f"m_{key_suffix}")
                pow_f = c3.number_input(f"Áram (Ft/perc)", value=float(all_settings[gep_nev]["power"]), format="%.4f", key=f"p_{key_suffix}")
                
                c4, c5, c6 = st.columns(3)
                work_f = c4.number_input(f"Munkadíj (Ft/perc)", value=float(all_settings[gep_nev]["work"]), format="%.2f", key=f"w_{key_suffix}")
                mag_f = c5.number_input(f"Szerelék (Ft/db)", value=float(all_settings[gep_nev]["magnet"]), format="%.2f", key=f"mag_{key_suffix}")
                paint_f = c6.number_input(f"Festék (Ft/mm²)", value=float(all_settings[gep_nev]["paint"]), format="%.6f", key=f"paint_{key_suffix}")
                
                if st.button(f"Alapértékek mentése ({gep_nev})", key=f"btn_{key_suffix}"):
                    save_settings(gep_nev, {"lazer": lazer_f, "material": mat_f, "power": pow_f, "work": work_f, "magnet": mag_f, "paint": paint_f})
            ...
            with col_a:
                ...
                g_time = st.number_input("Munkaidő (perc)", min_value=0.0, value=5.0, key=f"gt_{key_suffix}", help="Ide másold az SVG becslő eredményét!")
            ...
            with res2:
                st.write(f"Előállítási költség (+11%):")
                st.header(f"{round(eloallitasi_koltseg)} Ft")
            with res3:
                st.write(f"**Egy darab ára:**")
    ...
    # A két lézer fül legenerálása
    render_laser_tab("Kis Lézer", tabs[0], "kis")
    render_laser_tab("Nagy Lézer", tabs[1], "nagy")
    
    with tabs[2]:
        st.info("3D Nyomtatás kalkulátor - Fejlesztés alatt...")
        ...
        if uploaded_file:
            ...
            try:
                ...
                if width_match and viewbox_match:
                    ...
                    if unit == 'cm': phys_w *= 10
                    elif unit == 'pt': phys_w *= 0.3527
